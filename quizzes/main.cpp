...
int main(int argc, char *argv[]) {
   ...
   std::string ackerman_filename = "../a2/data/ACKERMAN.TXT";
   std::string quadcopter_filename = "../a2/data/QUADCOPTER.TXT";
 ...
 // indicating IDLE, we know it has been reached
    ...
    while (!OK){
        ...
        if(progress.size() != controllers.size()){
            std::cout << "Error: status size mismatch" << std::endl;
            break;
        }
    
        // Check if ALL platforms have completed (changed from && to all checking)
        ...
        if (allComplete) {
            std::cout << "Mission accomplished - all platforms complete" << std::endl;
            OK = true;
        } else {
    ...
    // Add a small delay before destroying objects
